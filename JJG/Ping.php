...
class Ping {
    ...
    private function pingExec() {
        ...
        if (!empty($output[1])) {
          ...
          //  $output[1] = '64 bytes from 192.168.101.17: icmp_seq=1 ttl=128 time=0.222 ms';
            //           'Respuesta desde 192.168.101.17: bytes=32 tiempo<1m TTL=128'
            $response = preg_match("/time(?:=|<)(?<time>[\.0-9]+)(?:|\s)ms/", $output[1], $matches);
         //   $response = preg_match("/tiempo(?:=|<)(?<tiempo>[\.0-9]+)(?:|\s)m/", $output[1], $matches);
            // If there's a result and it's greater than 0, return the latency.
            if ($response > 0 && isset($matches['time'])) {
                $latency = ceil($matches['time']);
            }
//            if ($response > 0 && isset($matches['tiempo'])) {
  //              $latency = ceil($matches['tiempo']);
  //          }
        }
